...
Most of the interesting design work here was in the extraction prompt. The schema is specific — I wanted dates as ISO strings, durations as integers, urgency as a strict enum — and Claude returns a raw JSON array with no markdown fences. There's a cleanup step to strip accidental fences anyway, but it rarely fires.
